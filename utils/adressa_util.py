...
def entities_addressa_dict(config):
    ...
    entities_dict = {value: index for index, value in enumerate(entities_set)}
...
def addressa_construct_adj(config, entities_dict, relations_dict):
    ...
    for key in tqdm(kg.keys()):
        ...
        for neighbor in neighbors:
            ...
            relation_adj[new_key].append(relation)
...
def obtain_train_test_adressa(config, train_adressa_behaviour, adressa_behaviours):
    ...
    if not (os.path.exists('./data/train/adr/adressa_train_news_dataset.tsv') or os.path.exists(
            './data/valid/adr/adressa_test_news_dataset.tsv')):
...
def build_news_addressa_features_mind(config, entity2embedd):
    ...
    if not (os.path.exists(embedding_folder + "train_news_addressa_embeddings.pkl") or os.path.exists(
    ...
    for i, news in enumerate(tqdm(news_feature_dict)):
        ...
        for item in entity_info_title:
            ...
            news_entity_feature[item['WikidataId']] = \
                (item['OccurrenceOffsets'], 1,
                 entity_type_dict[item['Type']])  # entity_freq, entity_position, entity_type
        ...
        # print(f'\nsentence embedding shape:{len(news_feature_list_ins[4])}')
        # print(news_feature_list_ins[4])
...
def build_user_history_adressa(config, train_adressa_behaviour, test_adressa_behaviour):
    ...
    for line in tqdm(train_adressa_behaviour):
        ...
        if len(permitted_urls) >= config['model']['user_his_num']:
            ...
            user_history_dict[user_id + "_train"] = permitted_urls + ["N0"] * (
                        config['model']['user_his_num'] - len(permitted_urls))
    ...
    for line in tqdm(test_adressa_behaviour):
        ...
        if len(permitted_urls) >= config['model']['user_his_num']:
            ...
            user_history_dict[user_id + "_dev"] = permitted_urls + ["N0"] * (
                        config['model']['user_his_num'] - len(permitted_urls))
...
def get_behavior_train_test(config, train_split_size=0.8):
    ...
    with open(config["data"]["train_adressa_behaviour"]) as behaviour:
        ...
        train_adressa_behaviour, test_adressa_behaviour = train_test_split(rows, train_size=train_split_size,
                                                                           random_state=42)
...
def load_data_mind_adressa(config):
    ...
    news_features, max_entity_freq, max_entity_pos, max_entity_type = build_news_addressa_features_mind(config,
                                                                                                        entity2embedd,
                                                                                                        entity_embedding)
